...
import {
  GtmDirective
} from "./chunk-SXRUW4O6.mjs";
import {
  GtmComponent,
  SeoService
} from "./chunk-OLAAPH4C.mjs";
import {
  EnumGtmEvent,
  GtmTracking,
  StateService,
...
function normalizeTrustProxyHeaders(trustProxyHeaders) {
  ...
  if (trustProxyHeaders === true) {
    return /* @__PURE__ */ new Set([TRUST_ALL_PROXY_HEADERS]);
  }
...
function requireNode() {
  ...
  Node2.prototype = Object.create(EventTarget.prototype, {
    ...
    compareDocumentPosition: { value: function compareDocumentPosition2(that) {
...
function requireElement() {
  ...
  Element2.prototype = Object.create(ContainerNode.prototype, {
    ...
    getElementsByTagName: { value: function getElementsByTagName2(lname) {
      var filter3;
    ...
    getElementsByClassName: { value: function getElementsByClassName2(names) {
  ...
  Attr.prototype = Object.create(Object.prototype, {
    ...
    cloneNode: { value: function cloneNode2(deep) {
      return new Attr(
        null,
        this.localName,
        this.prefix,
...
function requireCSSStyleDeclaration() {
  ...
  const { parse: parse2 } = requireStyle_parser();
...
function requireDocument() {
  ...
  Document2.prototype = Object.create(ContainerNode.prototype, {
    ...
    adoptNode: { value: function adoptNode(node2) {
    ...
    importNode: { value: function importNode(node2, deep) {
...
var ServerXhr = class _ServerXhr {
  ...
  async \u0275loadImpl() {
    if (!this.xhrImpl) {
      ...
      const {
        default: xhr
      } = await import("./xhr2-5UC6D7SJ.mjs");
      this.xhrImpl = xhr;
    }
  }
  build() {
  ...
  constructor() {
    ...
    if (config2.url) {
      const {
        ...
        href,
        origin
      } = resolveUrl(config2.url, this.origin);
...
function _document() {
  ...
  if (config2 && config2.document) {
    document2 = typeof config2.document === "string" ? _enableDomEmulation ? parseDocument(config2.document, config2.url !== void 0 ? resolveUrl(config2.url, "http://localhost").href : void 0) : window.document : config2.document;
  } else {
...
function requirePreviousMap() {
  ...
  let { dirname: dirname3, isAbsolute: isAbsolute2, join: join3, relative: relative2, sep: sep2 } = require$$2;
  ...
  class PreviousMap {
    ...
    loadFile(path2, cssFile, trusted) {
      if (!trusted && !this.unsafeMap) {
        ...
        let rel = relative2(dirname3(cssFile), path2);
        if (rel === ".." || rel.startsWith(".." + sep2) || isAbsolute2(rel)) {
          return void 0;
        }
      }
      this.root = dirname3(path2);
      if (existsSync(path2)) {
        this.mapFile = path2;
    ...
    loadMap(file, prev) {
      ...
      if (prev) {
        ...
        if (file) map2 = join3(dirname3(file), map2);
...
function requireInput() {
  ...
  let { isAbsolute: isAbsolute2, resolve: resolve3 } = require$$2;
  let { SourceMapConsumer, SourceMapGenerator } = require$$2;
  let { fileURLToPath: fileURLToPath2, pathToFileURL } = require$$2;
  let CssSyntaxError = requireCssSyntaxError();
  let PreviousMap = requirePreviousMap();
  let terminalHighlight = require$$2;
  let lineToIndexCache = /* @__PURE__ */ Symbol("lineToIndexCache");
  let sourceMapAvailable = Boolean(SourceMapConsumer && SourceMapGenerator);
  let pathAvailable = Boolean(resolve3 && isAbsolute2);
...
function requireMapGenerator() {
  ...
  let { dirname: dirname3, relative: relative2, resolve: resolve3, sep: sep2 } = require$$2;
  let { SourceMapConsumer, SourceMapGenerator } = require$$2;
  let { pathToFileURL } = require$$2;
  let Input = requireInput();
  let sourceMapAvailable = Boolean(SourceMapConsumer && SourceMapGenerator);
  let pathAvailable = Boolean(dirname3 && resolve3 && relative2 && sep2);
  class MapGenerator {
    ...
    applyPrevMaps() {
      for (let prev of this.previous()) {
        ...
        let root2 = prev.root || dirname3(prev.file);
        let map2;
...
function requireParser() {
  ...
  function tokensToString(tokens, from, to) {
    let result2 = "";
    for (let i = from; i < to; i++) result2 += tokens[i][1];
    return result2;
  }
  class Parser2 {
    constructor(input2) {
...
var safeParser = /* @__PURE__ */ getDefaultExportFromCjs2(safeParseExports);
var boolbase$1;
...
function serializeStylesheet(ast, options) {
  ...
  stringify(ast, (result2, node2, type) => {
    ...
    if (type === "end" && result2 === "}" && node2?.raws?.semicolon && (node2.type === "rule" || node2.type === "atrule")) {
      const lastChild = node2.nodes?.[node2.nodes.length - 1];
...
function buildCache(container2) {
  container2._classCache = /* @__PURE__ */ new Set();
  container2._idCache = /* @__PURE__ */ new Set();
...
function createDocument(html) {
  ...
  let beastiesContainers = document2.querySelectorAll("[data-beasties-container]");
  if (!beastiesContainers.length) {
    document2.documentElement?.setAttribute("data-beasties-container", "");
    beastiesContainers = [document2.documentElement || document2];
  }
  document2.beastiesContainers = beastiesContainers;
  for (const container2 of beastiesContainers) {
    buildCache(container2);
  }
  return document2;
}
...
function cachedQuerySelector(sel, node2) {
  ...
  if (selectorTokens && node2._classCache && node2._idCache) {
    for (const token of selectorTokens) {
      if (token.name === "class" && !node2._classCache.has(token.value)) {
        return false;
      }
      if (token.name === "id" && !node2._idCache.has(token.value)) {
        return false;
      }
    }
    return true;
...
var Beasties = class _Beasties {
  ...
  readFile(filename) {
    ...
    return new Promise((resolve3, reject) => {
      const callback = (err, data) => {
        if (err)
          reject(err);
        else resolve3(data.toString());
      };
      if (fs2 && fs2.readFile) {
        fs2.readFile(filename, callback);
      } else {
  ...
  writeFile(filename, data) {
    ...
    return new Promise((resolve3, reject) => {
      ...
      if (fs2 && fs2.writeFile) {
        fs2.writeFile(filename, data, callback);
      } else {
  ...
  async fetchStylesheet(link2, document2) {
    if (link2.hasAttribute("data-beasties-skip")) {
      return void 0;
    }
  ...
  processStyle(style, document2) {
    ...
    const beastiesContainers = document2.beastiesContainers;
    ...
    walkStyleRules(
      ...
      markOnly((rule2) => {
        if (rule2.type === "comment") {
          const beastiesComment = rule2.text.match(BEASTIES_COMMENT_RE);
        ...
        if (rule2.type === "rule") {
          ...
          rule2.filterSelectors?.((sel) => {
            ...
            if (!sel)
              return false;
            try {
              return beastiesContainers.some((container2) => container2.exists(sel));
            } catch (e) {
...
async function renderAngular(html, bootstrap, url, platformProviders, serverContext) {
  ...
  try {
    ...
    return {
      hasNavigationError,
      redirectTo,
      content: () => new Promise((resolve3, reject) => {
        setTimeout(() => {
          renderInternal(platformRef, applicationRef).then(resolve3).catch(reject).finally(() => void asyncDestroyPlatform2(platformRef));
        }, 0);
      })
    };
  } catch (error) {
...
${signal.reason}`, "AbortError"));
    ...
    promise.then(resolve3).catch(reject).finally(() => {
...
function extractRoutesAndCreateRouteTree(options) {
  const {
    url,
    manifest: manifest2 = getAngularAppManifest(),
  ...
  async function extract() {
    ...
    const document2 = await new ServerAssets(manifest2).getIndexServerHtml().text();
    const bootstrap = await manifest2.bootstrap();
    const {
      baseHref,
      appShellRoute,
      routes,
      errors
    } = await getRoutesFromAngularRouterConfig(bootstrap, document2, url, invokeGetPrerenderParams, includePrerenderFallbackRoutes, manifest2.entryPointToBrowserMapping);
...
var InlineCriticalCssProcessor = class extends BeastiesBase {
  ...
  constructor(readFile3, outputPath) {
    super({
      logger: {
...
var AngularServerApp = class {
  ...
  async handleRendering(request, matchedRoute, requestContext) {
    ...
    if (result2.redirectTo) {
      return createRedirectResponse(result2.redirectTo, responseInit.status, responseInit.headers);
    }
    if (renderMode === RenderMode.Prerender) {
...
import { dirname as dirname2, join as join2, normalize as normalize2, resolve as resolve2 } from "path";
...
var CommonEngineInlineCriticalCssProcessor = class {
  ...
  async process(html, outputPath) {
    const beasties = new InlineCriticalCssProcessor(async (path2) => {
      let resourceContent = this.resourceCache.get(path2);
      if (resourceContent === void 0) {
        resourceContent = await readFile2(path2, "utf-8");
...
var CommonEngine = class {
  ...
  inlineCriticalCss(html, opts) {
    const outputPath = opts.publicPath ?? (opts.documentFilePath ? dirname2(opts.documentFilePath) : "");
  ...
  async retrieveSSGPage(opts) {
    ...
    const pagePath = join2(publicPath, pathname, "index.html");
    if (this.pageIsSSG.get(pagePath)) {
      return fs.promises.readFile(pagePath, "utf-8");
    }
    if (!pagePath.startsWith(normalize2(publicPath))) {
      return void 0;
    }
    if (pagePath === resolve2(documentFilePath) || !await exists(pagePath)) {
      return void 0;
    }
  ...
  async renderApplication(opts) {
    ...
    if (!document2 && opts.documentFilePath) {
      document2 = await this.getDocument(opts.documentFilePath);
    }
...
var REQUEST2 = new InjectionToken("REQUEST Token");
var RESPONSE = new InjectionToken("RESPONSE Token");

// src/app/core/local.fn.ts
var LocalInterceptorFn = (req, next) => {
  const serverUrl = inject(SERVER_URL, { optional: true });
  const _req = inject(REQUEST2, { optional: true });
...
var AppRoutes = [
  ...
  __spreadValues({
    ...
    loadChildren: () => import("./project.route-5TPAYWFI.mjs").then((m) => m.ProjectRoutes),
    data: { preload: true, delay: 4e3 }
  }, true ? { \u0275entryName: "src/app/routes/project.route.ts" } : {}),
  __spreadValues({
    path: "products",
    component: MainLayoutComponent,
    loadChildren: () => import("./product.route-6XRIKH75.mjs").then((m) => m.ProductRoutes),
    data: { preload: true }
  }, true ? { \u0275entryName: "src/app/routes/product.route.ts" } : {}),
  // turn this into standalone
  __spreadValues({
    path: "content",
    component: SingleLayoutComponent,
    loadChildren: () => import("./content.route-VBNHKLI2.mjs").then((m) => m.ContentRoutes),
    data: { preload: true, delay: 1e3 }
  }, true ? { \u0275entryName: "src/app/routes/content.route.ts" } : {}),
  __spreadValues({
    path: "",
    component: SingleLayoutComponent,
    loadChildren: () => import("./public.route-JD5HHLH6.mjs").then((m) => m.PublicRoutes),
    data: { preload: true }
  }, true ? { \u0275entryName: "src/app/routes/public.route.ts" } : {}),
  {
    path: "**",
    redirectTo: "/404",
    // make 404
...
// server.ts
var _app = (context) => bootstrapApplication(AppComponent, {
  providers: [
    provideServerRendering(),
    ...appProviders
  ]
}, context);
var server_default = _app;
var engine = new CommonEngine({ bootstrap: _app, allowedHosts: ["localhost"] });
function crExpressEgine(filePath, options, callback) {
  try {
    const renderOptions = __spreadValues({}, options);
    renderOptions.documentFilePath = filePath;
    renderOptions.publicPath = options.settings?.views;
